...
class PlanRouteProblem(search.Problem):
    ...
    def h(self, node):
        ...
        P = list()
        for a in self.goals:
            P.append(manhattan_distance_with_heading(node.state, a))
            Q = min(P)
        return Q
    ...
    def actions(self, state):
        ...
        if state[2] == 0 and north in self.allowed:
            return ['Forward','TurnRight','TurnLeft']
        if state[2] == 1 and west in self.allowed:
                      
            return ['Forward','TurnRight','TurnLeft']

        if state[2] == 2 and south in self.allowed:
            
            return ['Forward','TurnRight','TurnLeft']
        if state[2] == 3 and east in self.allowed:
    ...
    def result(self, state, action):
        ...
        base_actions = ['Forward','TurnLeft', 'TurnRight']
        updated_state = state
        if action == base_actions[0]:
            if state[2] == 0:
                updated_state = (state[0],state[1] + 1,state[2])
                return updated_state
            if state[2] == 1:
                updated_state = (state[0]-1,state[1],state[2])
                return updated_state
            if state[2] == 2:
                updated_state = (state[0],state[1]-1,state[2])
                return updated_state
        ...
        if action == base_actions[2]:
            if state[2] == 0:
                updated_state = (state[0],state[1],3)
                return updated_state
            if state[2] == 1:
                updated_state = (state[0],state[1],0)
                return updated_state
            if state[2] == 2:
                updated_state = (state[0],state[1],1)
                return updated_state
            if state[2] == 3:
                updated_state = (state[0],state[1],2)
                return updated_state
        if action == base_actions[1]:
            if state[2] == 0:
                updated_state = (state[0],state[1],1)
                return updated_state
            if state[2] == 1:
    ...
    def goal_test(self, state):
        ...
        if (state[0],state[1]) in self.goals:
            return True
        if not (state[0],state[1]) in self.goals:
            return False
        ...
        for a in self.allowed:
            for b in self.goals:
                if (a[0] == b[0]) and (a[1] < b[1]):
                    shoot_result.append((a[0], a[1], 0))
                if (a[0] == b[0]) and (a[1] > b[1]):
                    shoot_result.append((a[0],a[1],2))
                if (a[0] > b[0]) and (a[1] == b[1]):
                    shoot_result.append((a[0], a[1], 1))
                if (a[0] < b[0]) and (a[1] < b[1]):
                    shoot_result.append((a[0], a[1], 3))
        if (state[0],state[1],state[2]) in shoot_result:
            return True
        else:
            return False
