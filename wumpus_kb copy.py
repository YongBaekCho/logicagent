...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for a in range(len(tvec)):
        if not tvec[a]:
            axiom_str += '~' + proposition_bases_perceptual_fluents[a] + time
        else:
            axiom_str += proposition_bases_perceptual_fluents[a] + time
        axiom_str += ' & '
    axiom_str = axiom_str[:-2]
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    "*** YOUR CODE HERE ***"
    Pit = pit_str(x,y)
    Wumpus = wumpus_str(x,y)
    axiom_str = '~{0} & ~{1}'.format(Pit, Wumpus)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    Pit = list()
    for a,b in [grid_1, grid_2, grid_3, grid_4, grid_initial]:

        if a >= xmin and a <= xmax and b >= ymin and b <= ymax:
            Pit.append(pit_str(a,b))
    
    axiom_str += '{0} <=> ({1})'.format(breezy,(' | ').join(Pit))
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for a,b in [grid_1, grid_2, grid_3, grid_4, grid_initial]:
        ...
        if a >= xmin and a <= xmax and b >= ymin and b <= ymax:
            Wumpi.append(wumpus_str(a,b))
    axiom_str += '{0} <=> ({1})'.format(stench,(' | ').join(Wumpi))
    return axiom_str
    ...
    for value in values:
        ...
        for x in values:
            if x != value:
                N.append(x)
        for y in N:
            senten = '~' + wumpus_str(y[0], y[1])
            M.append(senten)
        axioms.append('({0} ==> ({1}))'.format(wumpus_str(value[0],value[1]),' & '.join(M)))
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    for a in range(xmax+1):
        for b in range(ymax+1):
            if b == yi and a == xi:
                values.append(state_loc_str(a,b,t))
            else:
                values.append('~'+state_loc_str(a,b,t))
    axiom_str += ' & '.join(values)
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    for direction in ['North','South','East','West']:
        if direction.lower() == 'north':
            P.append('Heading' + direction + str(t))
            
        else:
            P.append('~Heading' + direction + str(t))
    axiom_str += '{0}{1}{2}'.format(direction, str(t),'&'.join(P))
    return axiom_str
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    a = state_have_arrow_str(t)
    b = state_wumpus_alive_str(t)
    axiom_str += a + ' & ' + b
    return axiom_str
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    a = state_OK_str(x,y,t)
    b = pit_str(x,y)
    c = state_wumpus_alive_str(t)
    d = wumpus_str(x,y)
    axiom_str = '{0} <=> (~{1} & ({3} >> ~{2}))'.format(a,b,c,d)
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    """
    axiom_str = ''
    a = state_loc_str(x,y,t)
    b = percept_breeze_str(t)
    c = breeze_str(x,y)
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    P = []
    if x-1 >= xmin: 
        P.append(state_loc_str(x-1,y,t))
    if x+1 <= xmax:
        P.append(state_loc_str(x+1,y,t))
    if y-1 <= ymax:
        P.append(state_loc_str(x,y-1,t))
    if y+1 <= ymax:
        P.append(state_loc_str(x,y+1,t))
    axiom_str += state_loc_str(x,y,t+1) + ' <=> ((' + '(' + ' | '.join(P) + ')' + ' & ' + action_forward_str(t) + ') | ' + '(' + state_loc_str(x,y,t) + ' & ~' + action_forward_str(t) + '))'
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    a = state_wumpus_alive_str(t)
    b = state_wumpus_alive_str(t+1)
    c = percept_scream_str(t+1)
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    a = state_heading_north_str(t)
    b = state_heading_north_str(t+1)
    c = action_turn_right_str(t)
    d = action_turn_left_str(t)
    e = state_heading_east_str(t)
    f = state_heading_west_str(t)
...
def axiom_generator_heading_east_ssa(t):
    ...
    axiom_str += state_heading_east_str(t+1) + ' <=> ' + ' ((' + e + ' & ' +' ~' +c + ' & ' + ' ~' + d + ')'\
                  ' | ' + ' (' + g + ' & ' + d + ' )' + ' | ' + ' (' + a + ' & ' + c + '))' 
...
def axiom_generator_heading_south_ssa(t):
    ...
    axiom_str += state_heading_south_str(t+1) + ' <=> ' + ' ((' + g + ' & ' +' ~' +d + ' & ' + ' ~' + c + ')'\
                  ' | ' + ' (' + f + ' & ' + d + ' )' + ' | ' + ' (' + e + ' & ' + c + '))' 
...
def axiom_generator_heading_west_ssa(t):
    ...
    axiom_str += state_heading_west_str(t+1) + ' <=> ' + ' ((' + f + ' & ' +' ~' +d + ' & ' + ' ~' + c + ')'\
                  ' | ' + ' (' + a + ' & ' + d + ' )' + ' | ' + ' (' + c + ' & ' + g + '))' 
...
def axiom_generator_heading_only_north(t):
    ...
    "*** YOUR CODE HERE ***"
    a = state_heading_north_str(t)
    b = state_heading_east_str(t)
    c = state_heading_west_str(t)
    d = state_heading_south_str(t)
...
def axiom_generator_only_one_action_axioms(t):
    ...
    "*** YOUR CODE HERE ***"
    proposition_bases_actions = ['Forward', 'Grab', 'Shoot', 'Climb',
                             'TurnLeft', 'TurnRight', 'Wait'] #from the above

    b1 = list()
    b = list()
    for i in range(len(proposition_bases_actions)):
        a = list()
        for j in range(len(proposition_bases_actions)):
            if j != i:
                a.append('~'+proposition_bases_actions[j] + str(t))
        b1.append('({0}{1} <=> ({2}))'.format(proposition_bases_actions[i],str(t),' & '.join(a)))
